...
vcf <- read.vcfR("~/Desktop/Documents/Research/Paper Code/Debban-Lamb/VCFs/vcf_reviews/mindp10_maf001_noTperf.recode.vcf", convertNA=T)
...
cutoff = 0.65
pca <- assess_missing_data_pca(vcf, popmap = linmap, thresholds=c(cutoff), clustering=T) # 0.85,0.80,0.75,0.70,0.65 ... 0.65 looks best
...
write.table(missing, "VCF_filtering/missing_list.txt", row.names = F, quote = F, col.names = F)
...
# # REPEAT MISSING EXCLUSION
# 
# new_cutoff = 0.6
# pca <- assess_missing_data_pca(vcf, popmap = pop_2, thresholds=c(new_cutoff), clustering=T) # 0.85,0.80,0.75,0.70,0.65 ... 
# # assess_missing_data_tsne(vcf, popmap = linmap, thresholds=c(cutoff)) # 0.85,0.80,0.75,0.70,0.65 ... 
# 
# # find real missing... not sure why it isn't working in SNPfiltR
# # vcf_cut <- missing_by_snp(vcf, cutoff=cutoff) 
# snps <- vcfR::extract.gt(vcf, element = "GT", IDtoRowNames  = F, as.numeric = T, convertNA = T, return.alleles = F)
# missing_sample <- colSums(is.na(snps))/dim(snps)[1]
# pca <- pca[[1]]
# pca$missing <- missing_sample
# pca$pop_sep <- pop_redux$pop
# 
# # and replot by missing... want to avoid high individual missing issues by removing those nearest to center
# ggplot(pca, aes(x=PC1, y=PC2, color=missing_sample))+
#   geom_point(size=5)+
...
# 
# pca <- pca %>% 
#   tidyr::separate(pop, into=c("lin", "reg"), sep="_")
# 
# ggplot(pca, aes(x=PC1, y=PC2, color=lin, shape=reg))+
#   geom_point(size=5)+
#   scale_color_manual(values=c("green4", "skyblue2", "purple3"))+
#   theme_bw()
# 
# counts of inds by pop
genlight2 <- vcfR2genlight(vcf) #convert to genlight
inds <- data.frame(ind=genlight2$ind.names)

inds <- inds %>%
  tidyr::separate(ind, into=c("pop", "ind"), sep="-")
table(inds$pop) %>% as.data.frame()

# write.vcf(vcf, "VCFs/vcf_reviews/noTperf_65miss_50indmiss.vcf") # no programs seem to be able to read in VCFs produced by vcfR

### population map
# generating pop map to accommodate dartR's pop and id conjoining
pop_linreg <- read.csv("./VCF/noTperf_popmap.txt", sep="\t")
pop_reg_save <- subset(pop_linreg, id %in% genlight2@ind.names) %>% 
  tidyr::separate(lin, into=c("lin", "reg"), sep="_") 

pop_reg <- pop_reg_save[,c("id", "pop")]
# pop_reg$id <- paste0(pop_reg$pop, "_", pop_reg$id)

write.table(pop_reg, "./VCF/vcf_popmap.txt", row.names=F, col.names=F, sep="\t", quote=F)
