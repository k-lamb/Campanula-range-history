...
def SC_2C(params, ns, pop_ids=None):
    ...
    fs.integrate([nu1, nu2], Tsc2, dt_fac=0.01, m=np.array([[0, m12_2], [m21_2, 0]]))
    fs.pop_ids = pop_ids
    return fs


def SC_2C_ae(params, ns, pop_ids=None):
    if pop_ids is not None and len(pop_ids) != 2:
        raise ValueError("pop_ids must be a list of two population IDs")
    nu1, nu2, nu_ae, Tae, Ts, Ts2, Tsc, Tsc2, m12, m21, m12_2, m21_2 = params
    sts = moments.LinearSystem_1D.steady_state_1D(ns[0] + ns[1])
    fs = moments.Spectrum(sts)
...
#SC_2C
param_SC_2C = ["nu1", "nu2", "Ts", "Ts2", "Tsc", "Tsc2", "m12", "m21", "m12_2", "m21_2"]
upper_bound_SC = [20, 20, 10, 10, 10, 10, 20, 20, 20, 20]
lower_bound_SC = [1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3]

#SC_2C_ae
param_SC_2C_ae = ["nu1", "nu2", "nu_ae", "Tae", "Ts", "Ts2", "Tsc", "Tsc2", "m12", "m21", "m12_2", "m21_2"]
upper_bound_SCae = [20, 20, 20, 10, 10, 10, 10, 10, 20, 20, 20, 20]
lower_bound_SCae = [1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3]

param_list = [param_SC_2C, param_SC_2C_ae]
upper_bound_list = [upper_bound_SC, upper_bound_SCae]
lower_bound_list = [lower_bound_SC, lower_bound_SCae]
model_list = [SC_2C, SC_2C_ae]
